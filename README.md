...
岡山県高梁市および同市巨瀬町の水田の減少の様子を1976年から2021年の期間で地図化。\
...
<figure>
  <img src="./work/data/Output/img/takahashi_ndvi_diff_binary.png" width="80%">
  <figcaption>2025年の高梁市における春(4月-5月)と夏(7月-8月)のNDVIの差分(閾値0.4)</figcaption>
</figure>

## 4. Result
### 1. 土地利用の変遷
1976年当時は広範囲に存在した水田が、2021年にかけて徐々に森林や畑などの別の土地利用に遷移していく様子が可視化された。
### 2. リスクマップの生成/モデル評価
機械学習モデルにより、高梁市内の「今後消失するリスクが高い水田」をヒートマップ形式で可視化することに成功した。
...
### 3. 衛星データとの比較
春夏のNDVI画像の差分を利用して作成した水田マップを2025年時点での暫定的な正解とし、リスクマップとの重ね合わせの結果、モデルが「消失リスク高」と判定したエリアは71%の一致率で水田消失を予測できていると分かった。
...
- **説明変数の改善** : 以下のような変数を追加することでモデルの改善を達成できる可能性があると考える。
    - ある水田の周囲にどれだけ水田があるかの半径Xメートル内の水田率の追加
    - 獣害リスクとして、ある水田の周囲にどれだけ森林があるかの半径Xメートル内の森林率の追加
    - 最寄りの水源からの標高差

- **衛星データを用いた高精度の分析**: 距離のみでの高精度な予測には限界を感じており、衛星画像データから得られる指標を説明変数に追加したモデルの精度向上が必要だと考える。
...
- **水生生物のオカレンスデータを組み合わせた分析**: 水田に生息する生物のSDM(Species Distributiuon Model)と組み合わせた分析。
    - 水田だけにフォーカスした議論ではなく、水田に生息する生物の生息圏がどのように変化するか可視化・予測でき、さらに生物多様性分野に踏み込んだ議論が可能になると思われる。
