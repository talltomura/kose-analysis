...
## Motivation
岡山県巨瀬町は私talltomuraの祖父が住んでいた土地であり、自然豊かな土地であった。
...
<!--
5. アマガエルを例としたSDMモデルを作成
    - 目的変数: アマガエルの出現データ
    - 説明変数: WroldClim/標高/3で作成した確率マップ
    - 詳細
        - 確率マップは各座標に変化リスクが確率で保存されている
        - 求めた確率を「変わる/変わらない」の二値ラベルに変換するための閾値を設定する(閾値は3種:0.3/0.5/0.7)
        - この閾値を変更した際にマップ(SDM予測)がどのように変化するかをFoliumでインタラクティブに可視化
-->
## Result
...
* **Future Work (Plan):**
    ...
    - WorldClim（気候データ）や標高データを説明変数に追加
