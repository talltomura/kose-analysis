...
- **衛星データをさらに用いた高精度の分析**: 距離のみでの高精度な予測には限界を感じており、衛星画像データから得られる指標を説明変数に追加したモデルの精度向上が必要。
    - 土地利用分類データより水田であると定義されているポリゴンの座標を用いて衛星データから指標を抽出。特定した地点の年代ごとの水田の変化(NDVI推移)で耕作放棄されているか継続しているかの判定 等...
